  ...
  try {
    ...
    app.querySelector('main').innerHTML = `
      <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold text-txt">Ausleih-Historie</h1>
          ...
          ${eintraege.map((e) => `
            <div class="bg-surface border border-border rounded-lg p-3 text-sm">
              <div class="flex justify-between items-start gap-2">
                <div class="font-medium text-txt">${escapeHtml(e.benutzer.voller_name)}${
                  e.externes_team_name
                    ? ` <span class="font-normal text-muted">für</span> ${escapeHtml(e.externes_team_name)}`
                    : ''
                }</div>
                ${e.ist_offen ? '<span class="text-xs px-2 py-0.5 rounded bg-surface-2 text-lent">offen</span>' : ''}
              </div>
              ...
              ${e.rueckgabe_kommentar ? `
                <div class="text-xs mt-1 text-txt-2 italic">„${escapeHtml(e.rueckgabe_kommentar)}"</div>` : ''}
              <div class="text-xs text-muted mt-1">
                Dauer: ${e.dauer_tage} Tag${e.dauer_tage === 1 ? '' : 'e'}
              </div>
            </div>`).join('')}
        </div>` : '<div class="text-center py-12 text-muted">Noch keine Ausleihen.</div>'}`;
  } catch (err) {
    app.querySelector('main').innerHTML =
