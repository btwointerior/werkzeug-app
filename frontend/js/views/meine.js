...
export async function renderMeine() {
  ...
  app.innerHTML = `
    <main class="max-w-3xl mx-auto pb-24 pt-4 px-4">
      <h1 class="text-2xl font-bold text-slate-900 mb-4">Meine Ausleihen</h1>
      <div class="bg-white rounded-lg shadow-sm p-4 mb-4 border border-slate-200">
        <label class="text-sm font-medium text-slate-700 mb-2 block" for="code-input">
          Maschinen-Code eingeben
        </label>
        <form id="code-form" class="flex gap-2">
          <input id="code-input" type="text" placeholder="z.B. M-0001"
                 class="flex-1 border border-slate-300 rounded-lg px-3 py-3 text-base uppercase">
  ...
  try {
    ...
    liste.innerHTML = ausleihen.map((a) => `
      <a href="#/m/${encodeURIComponent(a.maschine.maschinen_code)}"
         class="block bg-white rounded-lg shadow-sm p-4 mb-3 border border-slate-200 hover:bg-slate-50">
        <div class="flex justify-between items-start gap-3">
          <div>
            <div class="font-semibold text-slate-900">${escapeHtml(a.maschine.name)}</div>
